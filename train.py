...
from Vision_Transformer.datasets.cifar import *
from Vision_Transformer.model.vit import VisionTransformer
from Vision_Transformer.utils.utils import setup_device, accuracy, MetricTracker, TensorboardWriter
...
def train_epoch(epoch, model, data_loader, criterion, optimizer, lr_scheduler, metrics, device=torch.device('cpu')):
    ...
    for batch_idx, (batch_data, batch_target) in enumerate(data_loader):
        ...
        if batch_idx % 100 == 0:
            print("Train Epoch: {:03d} Batch: {:05d}/{:05d} Loss: {:.4f} Acc@1: {:.2f}, Acc@5: {:.2f}".format(epoch, batch_idx, len(data_loader), loss.item(), acc1.item(),
                                                                                                              acc5.item()))
...
def main():
    ...
    if config.checkpoint_path:
        state_dict = model.load_checkpoint(config.checkpoint_path)
    ...
    train_dataloader = CIFAR10DataLoader(data_dir=config.data_dir, split='train', image_size=config.image_size, batch_size=config.batch_size, num_workers=config.num_workers)
    valid_dataloader = CIFAR10DataLoader(data_dir=config.data_dir, split='val', image_size=config.image_size, batch_size=config.batch_size, num_workers=config.num_workers)
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=config.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=config.wd)
